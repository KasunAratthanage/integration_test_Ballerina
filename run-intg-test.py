...
def     get_latest_released_dist():
    ...
    print("====dist_downl_url===="+ dist_downl_url)
    print("====get_product path**********====" + str(get_product_file_path()))
...
def get_latest_stable_artifacts_api():
    ...
    """
    print("============latest_product_build_artifacts_api====================" + latest_product_build_artifacts_api)
    ...
    print("=======main_artifact_elements===========" + str(main_artifact_elements))
    for main_artifact in main_artifact_elements:
        print("==============main_artifact========="+ str(main_artifact))
        canonical_name_elements = main_artifact.getElementsByTagName("canonicalName")
        print("========canonical_name_elements========="+ str(canonical_name_elements))
        for canonical_name in canonical_name_elements:
            print("=======dist_name===========" + dist_name)
            print("=======canonical_name.firstChild===========" + str(canonical_name.firstChild.nodeValue))
            print("=====distname==="+ dist_name)
            if canonical_name.firstChild.nodeValue == dist_name + ".pom":
                # print("=======Inside POM======")
                # print("======parent_node============" + main_artifact.parentNode)
...
def get_latest_stable_dist():
    ...
    print("==========build_num_artifact====================" + build_num_artifact)
    ...
    print("=====dist_downl_url=====" + dist_downl_url)
    print("====get_product_file_path(======"+ str(get_product_file_path()))
    # urllib2.urlretrieve(dist_downl_url, str(get_product_file_path()))
    print("******get_product_file_path*****"+ str(get_product_file_path()))
    extract_path = get_product_file_path()
    print("****extract_path*****"+ str(extract_path))
    cp.extract_product(Path('/home/kasun/ballerinatestrepo/storage/ballerina-0.981.2-SNAPSHOT.zip'), workspace)
    # download_file(dist_downl_url, str(get_product_file_path()))
...
def main():
    try:
        ...
        # print("**********intg_module_path************" + intg_module_path)
